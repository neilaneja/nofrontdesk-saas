...
// Check-in: Escapia Guest Guides (online portal accessible via reservation info)
// âââââââââââââââââââââââââââââââââââââââââââââââââ
...
class EscapiaAdapter extends BasePMSAdapter {
  ...
  async searchReservations(lastName) {
    ...
    return edges.map(({ node: r }) => this.normalizeReservation({
      ...
      status: r.status,
      // Escapia may return a guestGuideUrl for their Guest Guides portal
      checkInFormUrl: r.guestGuideUrl || null,
      _raw: r,
    }));
  }
  ...
  buildCheckInUrl(reservation, propertyConfig) {
    // 1. Use URL from API response (Escapia Guest Guides)
    if (reservation.checkInFormUrl) return reservation.checkInFormUrl;

    // 2. Check raw data
    const raw = reservation.raw;
    if (raw?.guestGuideUrl) return raw.guestGuideUrl;

    // 3. Return null â NoFrontDesk built-in form will be used
    return null;
  }

  // ââââââââï¿½ï¿½ï¿½ Name Variants âââââââââ
