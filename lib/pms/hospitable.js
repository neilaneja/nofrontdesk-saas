...
class HospitableAdapter extends BasePMSAdapter {
  ...
  async authenticate() {
    ...
    // Hospitable uses Bearer token directly, no token exchange needed
  ...
  async searchReservations(lastName) {
    ...
    // Filter by last name (case-insensitive) and date window
  ...
  buildCheckInUrl(reservation, propertyConfig) {
    // Hospitable does not provide guest check-in URLs
    return null;
  }
