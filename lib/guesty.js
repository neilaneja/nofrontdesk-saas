...
// Cache recent search results to speed up repeat/rapid lookups (2-min TTL)
...
async function searchReservations(token, lastName) {
  // Check search cache first (speeds up repeat lookups within 2 min)
  const cacheKey = `search_${token.slice(-8)}_${lastName.toLowerCase().trim()}`;
  const cached = searchCache.get(cacheKey);
  if (cached) {
    console.log(`Search cache hit for "${lastName}"`);
    return cached;
  }
...
//
// Priority logic:
//   1. If a guest app name is configured on the property, ALWAYS build
//      the URL from that name. This avoids issues when Guesty's
//      reservation.guestAppUrl points to a deleted/wrong guest app.
//   2. Only fall back to reservation.guestAppUrl when NO app name is configured.
//   3. Last resort: build from 'default'.
// ─────────────────────────────────────────────
function buildCheckInFormUrl(reservation, guestyGuestAppNames) {
  // If a guest app name is explicitly configured, always use it
  // This prevents stale/wrong guestAppUrl from breaking the flow
  ...
  // No configured app name — fall back to Guesty's URL on the reservation
  if (reservation.guestAppUrl) {
    return reservation.guestAppUrl;
  }

  // Last resort: use 'default' app name
  const tokenPayload = `{{guest_app::default}}`;
  const base64Token = Buffer.from(tokenPayload).toString('base64');
  return `https://guest-app.guesty.com/r/${reservation._id}/${base64Token}`;
}
