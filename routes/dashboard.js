...
router.post('/dashboard/credentials', async (req, res) => {
  ...
  try {
    // If we have both ID and secret, test the credentials first
    const secretToTest = clientSecret ? clientSecret.trim() : null;

    if (secretToTest) {
      const { getGuestyToken } = require('../lib/guesty');
      try {
        await getGuestyToken(`test_${accountId}`, clientId.trim(), secretToTest);
      } catch (testErr) {
        ...
        return res.send(dashboardLayout({ company_name: req.session.companyName }, `
          ...
          <form method="POST" action="/dashboard/credentials" class="form-card">
            <div class="form-group">
              <label>Guesty Client ID</label>
              <input type="text" name="clientId" value="${esc(clientId)}" required>
            </div>
            <div class="form-group">
              <label>Guesty Client Secret</label>
              <input type="password" name="clientSecret" placeholder="Paste your client secret" required>
            </div>
    ...
    if (existing.rows.length > 0) {
      if (secretToTest) {
        await db.query(
          ...
          [clientId.trim(), secretToTest, accountId]
        );
      } else {
        await db.query(
      ...
      await db.query(
        ...
        [accountId, clientId.trim(), secretToTest]
      );
    }
