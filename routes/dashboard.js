...
function depositBadges(pid, psMap) {
  var ps = psMap[pid];
  if (!ps || Number(ps.total_payments) === 0) return '';
  var h = [];
  if (Number(ps.active_holds) > 0) h.push('<span style="display:inline-block;padding:2px 8px;border-radius:12px;font-size:11px;background:#fef3c7;color:#92400e">' + ps.active_holds + ' held</span>');
  if (Number(ps.completed) > 0) h.push('<span style="display:inline-block;padding:2px 8px;border-radius:12px;font-size:11px;background:#d1fae5;color:#065f46">' + ps.completed + ' paid</span>');
  if (Number(ps.released) > 0) h.push('<span style="display:inline-block;padding:2px 8px;border-radius:12px;font-size:11px;background:#e0e7ff;color:#3730a3">' + ps.released + ' released</span>');
  return h.length ? '<div style="font-size:12px;margin-top:4px">' + h.join(' ') + '</div>' : '';
}
...
router.get('/dashboard', async (req, res) => {
  ...
  const [accountRes, propertiesRes, credRes, statsRes, payStatsRes] = await Promise.all([
    ...
    db.query('SELECT property_id, COUNT(*) as total_payments, COUNT(CASE WHEN status = $held$ THEN 1 END) as active_holds, COUNT(CASE WHEN status IN ($succeeded$,$captured$,$paid$) THEN 1 END) as completed, COUNT(CASE WHEN status IN ($released$,$refunded$) THEN 1 END) as released FROM guest_payments WHERE account_id = $1 GROUP BY property_id', [accountId]).catch(function() { return { rows: [] }; }),
  ]);
  ...
  const psMap = {}; (payStatsRes.rows || []).forEach(function(r) { psMap[r.property_id] = r; });
  ...
  res.send(dashboardLayout(account, `
    ...
    <div class="property-card">
      ...
      <div class="property-actions">
        ...
        <a href="/dashboard/payments" class="action-link">Deposits</a>
